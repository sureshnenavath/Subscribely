...
class Command(BaseCommand):
    help = "Create an active Pro subscription for a given user"
    ...
    def handle(self, *args, **options):
        ...
        # Fetch plan
        try:
            plan = Plan.objects.get(name__iexact='Pro')
        except Plan.DoesNotExist:
            self.stdout.write(self.style.ERROR("Plan 'Pro' not found."))
            return

        # Subscription details
        now = timezone.now()
        days = plan.trial_days if getattr(plan, 'trial_days', None) else 14
        ...
        payment = Payment.objects.create(
            ...
            raw_response=json.dumps({
                'synthetic': True,
                'note': 'Created by management command create_pro_subscription',
        ...
        self.stdout.write(self.style.SUCCESS(
            f"✅ Created Pro subscription for user '{user.username}' "
