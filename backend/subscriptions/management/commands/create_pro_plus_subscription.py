...
class Command(BaseCommand):
    help = "Create an active Pro Plus subscription for a given user"
    ...
    def handle(self, *args, **options):
        ...
        # Fetch plan
        try:
            plan = Plan.objects.get(name__iexact='Pro Plus')
        except Plan.DoesNotExist:
            self.stdout.write(self.style.ERROR("Plan 'Pro Plus' not found."))
            return

        # Subscription details
        now = timezone.now()
        days = plan.trial_days if getattr(plan, 'trial_days', None) else 30
        ...
        payment = Payment.objects.create(
            ...
            raw_response=json.dumps({
                'synthetic': True,
                'note': 'Created by management command create_pro_plus_subscription',
        ...
        self.stdout.write(self.style.SUCCESS(
            f"✅ Created Pro Plus subscription for user '{user.username}' "
